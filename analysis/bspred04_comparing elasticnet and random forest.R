...
source(paste0(path_bs_repo,"/analysis/bspred01_recipe for model.R"))
...
saveRDS(glmnet_rs,paste0(path_bs_folder,"/working/glmnet_rs.RDS"))


source(paste0(path_bs_repo,"/analysis/bspred03_building random forest with tuning.R"))
saveRDS(rf_res,paste0(path_bs_folder,"/working/rf_res.RDS"))
...
# the last model
last_rf_mod <- 
  rand_forest(mtry = 4, min_n = 20, trees = 500) %>% 
  set_engine("ranger", 
             # num.threads = cores, 
             importance = "impurity") %>% 
...
last_rf_fit %>% 
  collect_metrics()
saveRDS(last_rf_fit,paste0(path_bs_folder,"/working/last_rf_fit.RDS"))
...
last_glmnet_fit %>% 
  collect_metrics()
saveRDS(last_glmnet_fit,paste0(path_bs_folder,"/working/last_glmnet_fit.RDS"))
...
saveRDS(last_rf_vip,paste0(path_bs_folder,"/working/last_rf_vip.RDS"))
