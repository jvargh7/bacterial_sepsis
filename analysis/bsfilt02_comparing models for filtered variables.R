...
source(paste0(path_bs_repo,"/analysis/bsfilt01_recipe from bivariate filtering.R"))
...
saveRDS(glmnet_rs,paste0(path_bs_folder,"/working/glmnet_rs_filt.RDS"))


source(paste0(path_bs_repo,"/analysis/bspred03_building random forest with tuning.R"))
saveRDS(rf_res,paste0(path_bs_folder,"/working/rf_res_filt.RDS"))
...
# the last model
last_rf_mod <- 
  rand_forest(mtry = 3, min_n = 30, trees = 1000) %>% 
  set_engine("ranger", 
             # num.threads = cores, 
             importance = "impurity") %>% 
...
last_rf_fit %>% 
  collect_metrics()
saveRDS(last_rf_fit,paste0(path_bs_folder,"/working/last_rf_fit_filt.RDS"))
...
last_glmnet_fit %>% 
  collect_metrics()
saveRDS(last_glmnet_fit,paste0(path_bs_folder,"/working/last_glmnet_fit_filt.RDS"))
...
saveRDS(last_rf_vip,paste0(path_bs_folder,"/working/last_rf_vip_filt.RDS"))
